...
class YaswapBoostERC20toNative extends SwapProvider {
  ...
  async updateOrder(order: BoostHistoryItem) {
    return await this.yaswapSwapProvider.updateOrder(order as YaswapSwapHistoryItem);
  }
  ...
  async finalizeAutomatedMarketMakerAndStartYaswapSwap({
    ...
    if (resultAMM?.status === 'SUCCESS') {
      const resultLSP = await this.yaswapSwapProvider.newSwap({
        network: _network,
        walletId: _walletId,
        quote: swapLSP,
      });
      ...
      return {
        ...resultLSP,
        ...swapLSP,
        toAmount: (resultLSP as YaswapSwapHistoryItem).toAmount,
  ...
  protected _getStatuses(): Record<string, SwapStatus> {
    return {
      ...
      // Yaswap swap states
      INITIATED: {
        ...this.yaswapSwapProvider.statuses.INITIATED,
        step: 2,
        label: 'Locking {bridgeAsset}',
      },
      INITIATION_REPORTED: {
        ...this.yaswapSwapProvider.statuses.INITIATION_REPORTED,
        step: 2,
        label: 'Locking {bridgeAsset}',
      },
      INITIATION_CONFIRMED: {
        ...this.yaswapSwapProvider.statuses.INITIATION_CONFIRMED,
        step: 2,
        label: 'Locking {bridgeAsset}',
      },
      ...
      WAITING_FOR_REFUND: {
        ...this.yaswapSwapProvider.statuses.WAITING_FOR_REFUND,
        step: 4,
      },
      GET_REFUND: {
        ...this.yaswapSwapProvider.statuses.GET_REFUND,
        label: 'Refunding {bridgeAsset}',
        step: 4,
      },
      WAITING_FOR_REFUND_CONFIRMATIONS: {
        ...this.yaswapSwapProvider.statuses.WAITING_FOR_REFUND_CONFIRMATIONS,
        label: 'Refunding {bridgeAsset}',
        step: 4,
      },
      // final states
      REFUNDED: {
        ...this.yaswapSwapProvider.statuses.REFUNDED,
        step: 5,
      },
      SUCCESS: {
      ...
      QUOTE_EXPIRED: {
        ...this.yaswapSwapProvider.statuses.QUOTE_EXPIRED,
        step: 5,
      },
      FAILED: {
  ...
  protected _timelineDiagramSteps(): string[] {
    ...
    const lspTimeline = this.yaswapSwapProvider.timelineDiagramSteps;
  ...
  protected _totalSteps(): number {
    let lspSteps = this.yaswapSwapProvider.totalSteps;
    if (this.yaswapSwapProvider.timelineDiagramSteps[0] === 'APPROVE') {
      lspSteps -= 1;
    }
