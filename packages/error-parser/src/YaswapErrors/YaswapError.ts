...
import { YASWAP_ERROR_STRING_STARTER } from '../utils';

const { PLAIN, PLACEHOLDER } = TRANSLATION_KEYS;
export abstract class YaswapError<Context extends JSONObject = JSONObject> extends Error {
  source: string;
  ...
  toString(): string {
    ...
    return `${YASWAP_ERROR_STRING_STARTER}${JSON.stringify(jsonifiedErrorWithStack)}`;
  }
}
